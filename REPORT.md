...
![Architectures: PWFO one-shot operator (top) and the recurrent flow-map stepper (bottom)](plots/results/r12_architecture.png)

The PWFO backbone (§7) is visible directly: the phase $\Phi(t)=\phi_0+\int\omega\,d\tau$ and
envelope $\psi(t)=\rho_0 e^{\int\kappa\,d\tau}$ are prefix-sums, so time enters only through
bounded periodic harmonics $\cos k\Phi$ and a decaying $\psi$.

![PWFO backbone: the accumulated phase is a prefix-sum (evaluable at any $t$), and $\cos k\Phi$ is bounded and periodic for all $t$](plots/results/r3_pwfo_concept.png)
...
![Hybrid routing: which surrogate answers a query, as a function of current speed and query horizon](plots/results/r11_hybrid_routing.png)

### 10.6 Speed

PWFO has ~33k parameters (core) / ~170k (general); one forward pass is $O(K)$ in the query
time (a shared profile prefix-sum $O(S)$, then gather + evaluate), and a batched far-horizon
query (32 trajectories × 7500 query times to $t=1500$) runs in ≈70 ms on a 6 GB GPU, with
identical cost at $t\approx 1$ and $t\approx 10^6$. The flow-map (~20k parameters) rolls 32
trajectories × 3 cycles (554 coarse steps) in ≈25 ms; its coarse step $\Delta=0.2$ is a real
speedup over a stiff fine solver and is the lever for Hodgkin–Huxley (§8.4, §12).
...
- `results_figures.py` — regenerates every figure in `plots/results/` used by this report.
...
python results_figures.py                                 # -> plots/results/
